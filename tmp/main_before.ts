...
import fs from "fs";
import Module from "module";import { verifyFromDisk, getMachineId } from "@/utils/license";
...
function getNodeModulesPaths(): string[] {
  const paths: string[] = [];
  if (app.isPackaged) {
    // external 渚濊禆锛堝師鐢熸ā鍧楋級鍦?unpacked 鐩綍
    const unpackedNodeModules = path.join(process.resourcesPath, "app.asar.unpacked", "node_modules");
    if (fs.existsSync(unpackedNodeModules)) {
      paths.push(unpackedNodeModules);
    }
    // 鏅€氫緷璧栧湪 asar 鍐?    const asarNodeModules = path.join(process.resourcesPath, "app.asar", "node_modules");
...
//鍔ㄦ€佸姞杞?function requireWithCustomPaths(modulePath: string): any {
  ...
  (Module as any)._nodeModulePaths = function (from: string): string[] {
    ...
    // 灏嗕富绋嬪簭鐨?node_modules 娣诲姞鍒板墠闈?    for (let i = appNodeModulesPaths.length - 1; i >= 0; i--) {
  ...
  try {
    // 娓呴櫎缂撳瓨纭繚鍔犺浇鏈€鏂?    delete require.cache[require.resolve(modulePath)];
